...
// so no other record type (AAAA, CNAME...) can point to one. ctx is the
// caller's, so a shutdown cuts the walk short.
func ListZoneARecords(ctx context.Context, client *dns.RecordService, zoneID string, zoneName string) ([]models.DNSRecord, error) {
	...
	iter := client.ListAutoPaging(ctx, params)
	for iter.Next() {
		record := iter.Current()
